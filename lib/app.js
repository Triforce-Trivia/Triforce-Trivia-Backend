...
app.get('/werewolves', async(req, res) => {
  try {
    // const difficulty = req.query.search; 
    const triviaData = await request.get('https://opentdb.com/api.php?amount=8&category=27&difficulty=medium&type=boolean'); 
    const mungedTriviaData = mungeTriviaFunc(triviaData.body); 
    res.json(mungedTriviaData);
  } catch(e) {
    res.status(500).json({ error: e.message });
...
app.get('/api/userscores/:id', async(req, res) => {
  try {
